...
class TokenPCStats(nn.Module):
    ...
    def begin_batch(self, global_token_ids: torch.Tensor) -> Optional[BatchUpdateContext]:
        ...
        unique_local_ids, inverse = torch.unique(local_token_ids, sorted=True, return_inverse=True)
        if unique_local_ids.numel() == 0:
            return None

        batch_counts = torch.bincount(inverse, minlength=unique_local_ids.numel()).to(dtype=torch.int64)
        old_counts = self.token_counts[unique_local_ids].clone()
        new_counts = old_counts + batch_counts
        self.token_counts[unique_local_ids] = new_counts

        return BatchUpdateContext(
            row_indices=row_indices,
            unique_local_ids=unique_local_ids,
            inverse=inverse,
    ...
    def update_layer_from_batch(
        ...
        if not finite_mask.all():
            ...
            local_ids = (ctx.inverse[finite_mask]).to(torch.int64)
            unique_local_ids = ctx.unique_local_ids
            batch_counts = torch.bincount(local_ids, minlength=unique_local_ids.numel()).to(torch.int64)
            old_counts = (ctx.new_counts - ctx.batch_counts)
            new_counts = old_counts + batch_counts
            inverse = local_ids
        else:
            unique_local_ids = ctx.unique_local_ids
            inverse = ctx.inverse
            batch_counts = ctx.batch_counts
            old_counts = ctx.old_counts
            new_counts = ctx.new_counts

        if x_local.numel() == 0:
            return

        x_local = x_local.to(dtype=self.stats_dtype)
        u = unique_local_ids.numel()
        d = x_local.shape[1]

        batch_sums = torch.zeros(u, d, device=x_local.device, dtype=self.stats_dtype)
        batch_sums.index_add_(0, inverse, x_local)
        batch_means = batch_sums / batch_counts.clamp_min(1).to(dtype=self.stats_dtype).unsqueeze(1)

        mean_table = self.layer_mean(layer_idx)
        pc_table = self.layer_pc(layer_idx)
        old_means = mean_table[unique_local_ids]
        old_pcs = pc_table[unique_local_ids]

        alpha = (batch_counts.to(dtype=self.stats_dtype) / new_counts.clamp_min(1).to(dtype=self.stats_dtype)).unsqueeze(1)
        new_means = old_means + alpha * (batch_means - old_means)
        mean_table[unique_local_ids] = new_means

        row_means = new_means[inverse]
        row_pcs = old_pcs[inverse]
        xc = x_local - row_means

        proj = (xc * row_pcs).sum(dim=1)
        weighted = xc * proj.unsqueeze(1)

        g = torch.zeros(u, d, device=x_local.device, dtype=self.stats_dtype)
        g.index_add_(0, inverse, weighted)

        candidate = g + (self.oja_eps * old_pcs)
        eligible = new_counts >= self.min_count_for_pc_update
        if eligible.any():
            eligible_idx = torch.nonzero(eligible, as_tuple=False).squeeze(1)
            cand_eligible = candidate[eligible_idx]
            norms = cand_eligible.norm(dim=1, keepdim=True)
            nonzero = norms.squeeze(1) > 0
            if nonzero.any():
                normed = cand_eligible[nonzero] / norms[nonzero].clamp_min(1e-12)
                write_idx = eligible_idx[nonzero]
                pc_table[unique_local_ids[write_idx]] = normed
...
def collect_w3_outputs(
    ...
    def _w3_hook(layer_idx: int):
        def hook_fn(module, inp, out):
            intermediates[layer_idx] = out.detach().to(dtype=collect_dtype)
        return hook_fn
...
def reduce_pc_stats_across_dp(stats: TokenPCStats, chunk_size: int):
    ...
    for start in range(0, vocab_local, chunk_size):
        ...
        count_f = count_chunk.to(dtype=stats.stats_dtype).unsqueeze(1)
        for layer_idx in stats.stem_layers:
            ...
            weighted_mean = mean_chunk * count_f
            weighted_pc = pc_chunk * count_f
...
def _save_local_pc_stats(stats: TokenPCStats, train_state: PCWarmupState, ckpt_dir: Path):
    ...
    torch.save(payload, ckpt_dir / f"pc_stats_rank{rank:05d}.pt")
    if world_size == 1:
...
def train(args: ProjectionPCWarmupArgs):
    with ExitStack() as context_stack:
        ...
        while train_state.step < args.steps:
            ...
            intermediates, nll = collect_w3_outputs(
                model,
                input_ids,
                stem_layers,
            ...
            if every_n_steps(train_state, args.logging.freq, acc_step=0, acc_freq=False):
                ...
                to_sync: Dict[str, float] = {
                    "loss/nll_original": float(nll.item()) if nll is not None else 0.0,
                }
                ...
                if get_is_master():
                    ...
                    logger.info(
                        ...
                        f"  nll: {synced_losses['loss/nll_original']:.4f}"
                        f"  wps: {wps:.2e}"
    ...
    gc.collect()
    logger.info("Projection PC warmup complete.")
