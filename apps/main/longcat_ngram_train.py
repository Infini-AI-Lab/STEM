...
# tables, dual optimizers, :class:`lingua.stem_checkpoint.StemCheckpointManager`).
...
def train(args: LongcatTrainArgs):
    with ExitStack() as context_stack:
        ...
        # Ensure ngram_embeddings parameters require gradients and verify they're on the correct device
        for i, embedding in enumerate(model.ngram_embeddings):
            # Verify device after to_empty()
            weight_device = embedding.weight.device
            if weight_device.type != "cuda":
                logger.warning(
                    f"ngram_embeddings[{i}].weight is on {weight_device} after to_empty(), "
        ...
        # Verify ngram_embeddings are initialized after init_weights()
        for i, embedding in enumerate(model.ngram_embeddings):
            for param_name, param in embedding.named_parameters():
                if param.numel() > 0:
                    ...
                    logger.info(
                        f"ngram_embeddings[{i}].{param_name}: "
                    ...
                    if is_zero and not getattr(
                        ...
                        logger.error(
                            f"ERROR: ngram_embeddings[{i}].{param_name} is still all zeros after init_weights()!"
                        )
        ...
        if args.checkpoint.init_ckpt_path:
            ...
            # ``stem_shards/`` holds vocabulary-parallel shards (STEM tables or Longcat n-gram).
