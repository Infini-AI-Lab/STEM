...
from apps.main.stem_iir import (
    IIRStemLMTransformerArgs,
    IIR_STEM_MODEL_REGISTRY,
)

# Register IIR model types so that launch_stem_eval can resolve them.
...
@torch.no_grad()
def _load_iir_params(model, stem_shards_dir: Path):
    """Load IIR parameters from ``iir_params.pt`` inside *stem_shards_dir*.

    If the file doesn't exist (e.g. loading from a base STEM checkpoint
    without IIR preparation), falls back to ``reset_parameters()`` which
    uses the values from the model config.
    """
    iir_path = stem_shards_dir / IIR_PARAMS_FILE
    if iir_path.exists():
        iir_sd = torch.load(iir_path, map_location="cpu")
        for i, iir_mem in enumerate(model.iir_memories):
            prefix = f"iir_memories.{i}"
            for pname, param in iir_mem.named_parameters():
                key = f"{prefix}.{pname}"
                if key in iir_sd:
                    param.data.copy_(iir_sd[key].to(param.device))
                    logger.info(f"Loaded {key}: {param.data.flatten()[:4].tolist()}")
                else:
                    logger.warning(f"{key} not found in {iir_path}")
            for bname, buf in iir_mem.named_buffers():
                key = f"{prefix}.{bname}"
                if key in iir_sd:
                    buf.data.copy_(iir_sd[key].to(buf.device))
                    logger.info(f"Loaded {key}: {buf.data.flatten()[:4].tolist()}")
        logger.info(f"Loaded IIR parameters from {iir_path}")
    else:
        logger.info(
            f"No {IIR_PARAMS_FILE} in {stem_shards_dir}, "
            "falling back to reset_parameters() from model config"
        )
        for iir_mem in model.iir_memories:
            iir_mem.reset_parameters()


def sync_stem_params_across_dp(model):
    """Broadcast stem_embeddings and iir_memories weights from dp_rank 0 to all
    STEM data-parallel ranks.

    This ensures all DP ranks start with identical non-FSDP parameters, which
    is required because FSDP-sharded lm_transformer init may consume different
    amounts of RNG on different ranks, causing divergence.

    Must be called after any stem/iir parameter initialization or reset.
    """
    ...
    logger.info("Synchronized stem_embeddings and iir_memories weights across STEM data-parallel ranks")
...
@dataclass
class IIRStemTrainArgs(TrainArgs):
    model: IIRStemLMTransformerArgs = field(default_factory=IIRStemLMTransformerArgs)

    # Separate learning rate and weight decay for stem parameters
    # (stem_embeddings + iir_memories).
...
def train(args: IIRStemTrainArgs):
    with ExitStack() as context_stack:
        ...
        if args.model_type not in IIR_STEM_MODEL_REGISTRY:
            raise ValueError(
                f"Unknown model_type '{args.model_type}'. "
                f"Available: {list(IIR_STEM_MODEL_REGISTRY.keys())}"
            )
        (
            stem_model_cls, _stem_args_cls,
            _build_fsdp_plan,
            _get_no_recompute_ops, _get_num_flop_per_token,
        ) = IIR_STEM_MODEL_REGISTRY[args.model_type]
        logger.info(f"Using IIR-STEM model type: {args.model_type} ({stem_model_cls.__name__})")
        ...
        # Ensure non-FSDP parameters (stem_embeddings + iir_memories) require gradients
        for name, param in model.stem_embeddings.named_parameters():
            param.requires_grad = True
        for name, param in model.iir_memories.named_parameters():
            param.requires_grad = True
        logger.info("Ensured stem_embeddings and iir_memories parameters require gradients")
        ...
        # Verify iir_memories after init
        for i, iir_mem in enumerate(model.iir_memories):
            for param_name, param in iir_mem.named_parameters():
                logger.info(
                    f"iir_memories[{i}].{param_name}: "
                    f"device={param.device}, shape={param.shape}, "
                    f"value={param.data.flatten()[:4].tolist()}"
                )
        ...
        # Optimizer for stem_embeddings + iir_memories (regular Tensors)
        ...
        if args.checkpoint.init_ckpt_path:
            ...
            # Load IIR parameters from checkpoint (or fall back to config defaults)
            stem_shards_dir = Path(args.checkpoint.init_ckpt_path) / "stem_shards"
            _load_iir_params(model, stem_shards_dir)
        ...
        while train_state.step < args.steps:
            ...
            if train_state.acc_step == 0:
                ...
                # Clip stem + iir gradients (regular Tensors)
                stem_params = [p for p in model.stem_parameters() if p.grad is not None]
                if stem_params:
                    stem_grad_norm = torch.nn.utils.clip_grad_norm_(
                        stem_params, max_norm=args.optim.clip, foreach=False
                    ).item()

                # Sync non-FSDP gradients across STEM data-parallel ranks
                if get_stem_data_parallel_world_size() > 1:
                    dp_group = get_stem_data_parallel_group()
                    for param in model.stem_parameters():
            ...
            if every_n_steps(
                ...
                # Log IIR parameters (mu, alpha) per stem layer
                iir_dict = {}
                for layer_idx, iir_mem in enumerate(model.iir_memories):
                    mu_val = iir_mem.mu.item()
                    alpha_val = iir_mem.alpha
                    if alpha_val.numel() == 1:
                        alpha_val = alpha_val.item()
                    else:
                        alpha_val = alpha_val.mean().item()
                    iir_dict[f"optim/iir_mu_{layer_idx}"] = mu_val
                    iir_dict[f"optim/iir_alpha_{layer_idx}"] = alpha_val
                ...
                if iir_dict:
                    mu_strs = [f"L{k.split('_')[-1]}={v:.4f}"
                               for k, v in iir_dict.items() if "mu" in k]
                    log_msg += f"  iir_mu: [{', '.join(mu_strs)}]"
                if alpha_dict:
...
def main():
    ...
    default_cfg = OmegaConf.structured(IIRStemTrainArgs())
