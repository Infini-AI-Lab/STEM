...
project_name="stem"
experiment_name="lm1b-full-lorafine"
NNODES=4
...
torchrun --nproc-per-node=8 --nnodes=${NNODES} -m apps.main.lora_finetune \
    config=apps/main/configs/lora_finetune.yaml \
    data.root_dir=/dev/shm \
    dump_dir=/checkpoints-fsx/beidchen-sandbox/STEM/logs/lm1b-full-lorafine \
    'lora_target_modules=[ffn_w3]' \
