...
class LMBackend:
    def __init__(
        ...
        self.model_forward = lambda model, x, input_pos, kv_append_indptr, kv_page_indices, kv_page_indptr, kv_page_lastlen: model(
            x,
            input_pos,
            kv_append_indptr,
            kv_page_indices,
        ...
        self.prefill = lambda model, x, input_pos, kv_append_indptr, kv_page_indices, kv_page_indptr, kv_page_lastlen: model.prefill(
            x,
            input_pos,
            kv_append_indptr,
            kv_page_indices,
    ...
    def load_model(
        ...
        self.model: Transformer = load_model(
    ...
    def setup_caches(self, max_batch_size: int = 1, max_seq_length: int = 2048):
        ...
        self.num_pages_per_request = torch.zeros(
            max_batch_size, device=self.device, dtype=torch.int32
        )
        self.page_size = page_size
        self.max_num_pages = max_num_pages

        # Init Target Attention Backend(Flashinfer)
        self.prefill_buffer = torch.empty(
            128 * 1024 * 1024, dtype=torch.uint8, device=self.device
        )
        self.prefill_wrapper = flashinfer.BatchPrefillWithPagedKVCacheWrapper(
            self.prefill_buffer, "NHD"
        )
        ...
        self.decode_buffer = torch.empty(
            128 * 1024 * 1024, dtype=torch.uint8, device=self.device
        )
        ...
        self.paged_kv_indices = torch.empty(
            max_num_pages, dtype=torch.int32, device=self.device
        )
        self.paged_kv_last_page_len = torch.zeros(
            (max_batch_size), dtype=torch.int32, device=self.device
        )
        self.decode_wrapper = flashinfer.BatchPrefillWithPagedKVCacheWrapper(
            self.decode_buffer,
            "NHD",
            use_cuda_graph=True,
    ...
    def compile(self, compile_prefill: bool = False):
        ...
        self.model_forward = torch.compile(
            self.model_forward, mode="max-autotune", fullgraph=True
        )
        if compile_prefill:
    ...
    def sample(self, logits: torch.Tensor):
        if self.model.process_group is not None:
            pg = self.model.process_group
            ...
            local_max_indices = local_max_indices + rank * local_vocab_size  # global vocab id

            # Gather local maxima from all TP ranks
            gathered_vals = [torch.empty_like(local_max_values) for _ in range(world_size)]
            gathered_idx  = [torch.empty_like(local_max_indices) for _ in range(world_size)]
            dist.all_gather(gathered_vals, local_max_values, group=pg)
            dist.all_gather(gathered_idx,  local_max_indices, group=pg)

            # [TP, B]
            vals = torch.stack(gathered_vals, dim=0)
            idxs = torch.stack(gathered_idx,  dim=0)

            # For each batch element, find which rank had the best max logit
            winner_rank = torch.argmax(vals, dim=0)  # [B]

            # Select the corresponding global vocab index
            next_token_ids = idxs.gather(0, winner_rank.unsqueeze(0)).squeeze(0)  # [B]
        else:
            # Non-TP case
    ...
    def pre_encode(self, dec_len):
        ...
        qo_indptr = self.qo_indptr * dec_len
        self.paged_kv_indices = torch.cat(
            [
                torch.arange(
                    i * self.max_num_pages_per_request,
                    i * self.max_num_pages_per_request + self.num_pages_per_request[i],
                    dtype=torch.int32,
                    device=self.device,
                )
        ...
        self.paged_kv_indptr[1:] = torch.cumsum(
            self.num_pages_per_request, dim=0, dtype=torch.int32
        )
        self.paged_kv_last_page_len = torch.full(
            (self.batch_size,), dec_len, dtype=torch.int32, device=self.device
        )
